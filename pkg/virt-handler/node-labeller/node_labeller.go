...
func (n *NodeLabeller) prepareLabels(node *v1.Node) map[string]string {
	...
	if n.hasTSCCounter() {
		newLabels[kubevirtv1.CPUTimerLabel+"tsc-frequency"] = fmt.Sprintf("%s", n.virtCaps.NodeTscInfo.Frequency)
		newLabels[kubevirtv1.CPUTimerLabel+"tsc-scalable"] = fmt.Sprintf("%t", n.virtCaps.NodeTscInfo.Scalable == "yes")
	}
